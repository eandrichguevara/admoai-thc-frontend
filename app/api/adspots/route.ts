...
const allowedStatuses: AdStatus[] = [
  "active",
  "paused",
  "deactivated",
  "pending",
...
export async function POST(req: Request) {
  ...
  if (!placement || typeof placement !== "string" || !allowedPlacements.includes(placement as AdPlacement)) {
    return NextResponse.json({ error: `Missing or invalid 'placement' (allowed: ${allowedPlacements.join(",")})` }, { status: 400 });
  }

  const finalStatus: AdStatus = allowedStatuses.includes(status as AdStatus) ? (status as AdStatus) : "active";
